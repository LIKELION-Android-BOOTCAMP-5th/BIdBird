...
_StatusInfo _mapAuctionTradeStatus(int? auctionCode, int? tradeCode) {
  ...
  if (tradeCode != null) {
    if (tradeCode == TradeStatusCode.paymentRequired) {
  ...
  switch (auctionCode) {
    ...
    case AuctionStatusCode.ready: // 경매 대기
    case AuctionStatusCode.inProgress: // 경매 진행 중
    case AuctionStatusCode.instantBuyPaymentPending: // 즉시 구매 진행 중
      return _StatusInfo('판매중', tradeBidPendingColor);

    // 낙찰/구매 완료/유찰 등 → 거래 종료
    case AuctionStatusCode.bidWon: // 낙찰
    case AuctionStatusCode.instantBuyCompleted: // 즉시 구매 완료
    case AuctionStatusCode.failed: // 유찰
      return _StatusInfo('거래 종료', tradeSaleDoneColor);
