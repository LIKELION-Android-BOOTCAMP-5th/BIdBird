...
class FilteredTradeListScreen extends StatefulWidget {
  ...
  @override
  State<FilteredTradeListScreen> createState() =>
      _FilteredTradeListScreenState();
}
...
class _FilteredTradeListScreenState extends State<FilteredTradeListScreen> {
  ...
  Widget _buildWithViewModel(BuildContext context) {
    ...
    try {
      existingViewModel = Provider.of<CurrentTradeViewModel>(
        context,
        listen: false,
      );
    } catch (e) {
  ...
  Widget _buildContent(
    ...
    if (_cachedSaleItems != null) {
      ...
      saleItems = widget.isSeller == null || widget.isSeller == true
          ? saleHistory
                .where(
                  (item) => TradeItemFilter.shouldIncludeSaleItem(
                    item,
                    targetActionTypes,
                  ),
                )
    ...
    if (_cachedBidItems != null) {
      ...
      bidItems = widget.isSeller == null || widget.isSeller == false
          ? bidHistory
                .where(
                  (item) => TradeItemFilter.shouldIncludeBidItem(
                    item,
                    targetActionTypes,
                  ),
                )
    ...
    if (_cachedTotalItems != null &&
        _cachedItemsByActionType == itemsByActionType) {
      totalItems = _cachedTotalItems!;
    } else {
      totalItems = itemsByActionType.values.fold<int>(
        0,
            ...
            : TransparentRefreshIndicator(
                ...
                child: Builder(
                  builder: (context) {
                    ...
                    final sortedActionTypes = [...targetActionTypes];
                    sortedActionTypes.sort((a, b) {
                      const typeOrder = {
                      ...
                      return (typeOrder[a] ?? 999).compareTo(
                        typeOrder[b] ?? 999,
                      );
                    });

                    // 액션 타입별로 섹션 생성
                    for (final actionType in sortedActionTypes) {
                      ...
                      final saleCount = items
                          .whereType<SaleHistoryItem>()
                          .length;
                      ...
                      final fontSize = ResponsiveConstants.fontSizeLarge(
                        context,
                      );
                      allSections.add(
                        Padding(
                      ...
                      for (var i = 0; i < items.length; i++) {
                        ...
                        final bidItem = itemIsSeller
                            ? null
                            : item as BidHistoryItem;
                        final itemActionType =
                            saleItem?.actionType ??
                            bidItem?.actionType ??
                            TradeActionType.none;
                        // 섹션 마지막 아이템일 때 다음 섹션 헤더와 간격 확보
                        final bottomPadding = i == items.length - 1
                            ? spacing * 2.0
                            : spacing * 1.5;
                    ...
                    if (_displayedItemCount == 0 ||
                        _displayedItemCount > allSections.length) {
                    ...
                    final displaySections = allSections
      ...
      if (hasShippingInfo) {
        showDialog(
          ...
          builder: (dialogContext) => ShippingInfoViewPopup(
            createdAt: shippingInfo['created_at'] is String
                ? shippingInfo['created_at'] as String
                : null,
            carrier: shippingInfo['carrier'] is String
        ...
        showDialog(
          ...
          builder: (dialogContext) {
            return ShippingInfoInputPopup(
              initialCarrier:
                  shippingInfo != null && shippingInfo['carrier'] is String
                  ? shippingInfo['carrier'] as String
                  : null,
              initialTrackingNumber: trackingNumber is String
                  ? trackingNumber
                  : null,
