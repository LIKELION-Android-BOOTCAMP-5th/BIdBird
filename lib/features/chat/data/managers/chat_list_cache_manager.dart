...
class ChatListCacheManager {
  ...
  /// ChattingRoomEntity 리스트에서 sellerId를 캐시에 저장
  /// itemId를 사용하여 데이터베이스에서 sellerId를 가져와 캐시에 저장
  Future<void> cacheSellerIds(List<ChattingRoomEntity> rooms) async {
    if (rooms.isEmpty) return;
    
    // 중복 제거된 itemId 목록
    final itemIds = rooms.map((room) => room.itemId).toSet().toList();
